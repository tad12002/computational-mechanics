...
# Project: Predicting the Stock Prices of General Motors
...
```{code-cell} ipython3
#Load pandas data frame for given NYSE data
data = pd.read_csv('../data/nyse-data.csv')
data['date'] = pd.to_datetime(data['date'])
data
```
...
```{code-cell} ipython3
#Pull Out Data only regarding GM
gm_data = data[data['symbol'] == 'GM']
...
Analyzing the data frame, looking from halfway through 2012 to 2014 the stock had a steady increase in price leading from 20 to 40 dollar showing a double in opening price in a span of under 2 years. This is then where the stock stayed at a slow decline having some bounce backs in 2015 and 2016 osciallting in a deacreasing manner. Using this steady rate of increase I will see if the model can predict how the increase would reflect going from 2012.5 to 2013.5 then predicting for 2014.
...
```{code-cell} ipython3
#First pick the data range to be looking for
gm_data_2012_2014 = gm_data[(gm_data['date'] > pd.to_datetime('2012-07-01')) & (gm_data['date'] < pd.to_datetime('2013-10-01'))]
```

```{code-cell} ipython3
plt.figure(figsize=(14, 7))
plt.plot(gm_data_2012_2014['date'], gm_data_2012_2014['open'])
plt.xlabel('date')
plt.ylabel('opening price (\$)');
```

```{code-cell} ipython3
dprice = np.diff(gm_data_2012_2014['open'])

plt.figure(figsize=(14, 7))
plt.plot(gm_data_2012_2014['date'][1:], dprice)
plt.xlabel('date')
plt.ylabel('change in opening price (\$/day)');
```
...
plt.hist(dprice, 50, density=True)
plt.plot(x, price_pdf)
plt.title('GM changes in price over 1.5 years\n'+
         'avg: \${:.2f} stdev: \${:.2f}'.format(mean_dprice, std_dprice));
```
...
rng = default_rng(42)
N_models = 100
dprice_model = rng.normal(size = (len(gm_data_2012_2014), N_models), loc = mean_dprice, scale = std_dprice)
...
```{code-cell} ipython3
price_model = np.cumsum(dprice_model, axis = 0) + gm_data_2012_2014['open'].values[0]


plt.figure(figsize=(14, 7))
plt.plot(gm_data_2012_2014['date'], price_model, alpha = 0.3);

plt.plot(gm_data_2012_2014['date'], gm_data_2012_2014['open'], c = 'k', label = 'NYSE data')
plt.xlabel('date')
plt.ylabel('opening price (\$)');
```
...
skip = 100
plt.errorbar(gm_data_2012_2014['date'][::skip], price_model_avg[::skip],
             yerr = price_model_std[::skip], 
             fmt = 'o',
             c = 'r', 
             label = 'model result', 
...
Yes the result does predict trend within the given data, however a numerical like so does not account for outside influences that are immeasurable and based on circumstance. This predictor is purely numerical based. For example if I were to do the data analysis past 2014 and or before 2012, the error would very much not represent the spread comparing prediction versus real values. The error would be very large and not be usable at all for predicting.

+++

## 5. Changing Date Size Frame for Science

```{code-cell} ipython3
#First pick the data range to be looking for
gm_data_2012_2014 = gm_data[(gm_data['date'] > pd.to_datetime('2010-07-01')) & (gm_data['date'] < pd.to_datetime('2014-10-01'))]
```
...
Looking at the result in 2013-05 error bar, you can see the spread is from around 19 to 43 where as the spread done from the limited time I did in the previous calculation had an error spread at 2013-05 being from 25 to 37. This would be a 24 spread versus a 12 spread. So adding in the whole data range doubles the error predictor at the same month. Again this is due to the predictor unable to predict non numerical influences on stock price. This maybe more useful for something like unstable crypto stock at a very small time frame.
